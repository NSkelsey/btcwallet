...
func SendBulletin(w *Wallet, chainSrv *chain.Client, icmd btcjson.Cmd) (interface{}, error) {
	...
	bltn, err := ahimsa.NewBulletinFromStr(cmd.Address, cmd.Board, cmd.Message)
	if err != nil {
		return nil, err
	}
	txouts, err := bltn.TxOuts(derp.DustAmnt(), activeNet.Params)
	if err != nil {
		return nil, err
	}
